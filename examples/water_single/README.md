...
Training a perturbative energy model is a pure machine-learning task. Please see the [DeePHF paper](https://arxiv.org/pdf/2005.00169.pdf) for a detailed explanation of the descriptors. The `deepks scf` command can run the Hartree-Fock calculation and save `descriptor` together with the supervised labels `e_corr_target` and `f_corr_explicit_target`; `e_corr` and `f_corr_explicit` denote the corresponding model-computed quantities, and `f_reference_variational` is the variational reference partition of the total force. The `deepks train` command uses the dumped descriptors and labels to train a neural-network model.

...
def test_gradient_driver_preserves_every_spin_and_motion_partition(
    uhf_oracle_case,
):
    driver = uhf_oracle_case.gradient_driver

    np.testing.assert_allclose(
        driver.dq_dR_relaxed_spin,
        driver.dq_dR_explicit_spin + driver.dq_dR_response_spin,
        rtol=0.0,
        atol=2.0e-13,
    )
    np.testing.assert_allclose(
        driver.dq_dR_explicit,
        driver.dq_dR_explicit_spin.sum(axis=0),
        rtol=0.0,
        atol=2.0e-13,
    )
    np.testing.assert_allclose(
        driver.dq_dR_response,
        driver.dq_dR_response_spin.sum(axis=0),
        rtol=0.0,
        atol=2.0e-13,
    )
    np.testing.assert_allclose(
        driver.dq_dR_relaxed,
        driver.dq_dR_relaxed_spin.sum(axis=0),
        rtol=0.0,
        atol=2.0e-13,
    )
    np.testing.assert_allclose(
